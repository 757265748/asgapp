	...
	<view class="index-content">
		...
		<view class="uni-tab-bar">
			...
			<!-- <uni-popup :show="type === 'middle-list'" position="middle" mode="fixed" @hidePopup="togglePopup('')">
				<view class="hide-more" @click="togglePopup('')">
				...
				<view class="page-body" :style="'height:'+height+'px'">
					...
					<scroll-view class="nav-right" scroll-y :scroll-top="scrollTop" @scroll="scroll" :style="'height:'+height+'px'"
					 scroll-with-animation>
						<view :id="index===0?'first':''" @tap="tabClick_more(item)" class="nav-right-item" v-for="(item,index) in subCategoryList"
						 :key="index">
							<image :src="item.LOGO" />
							<view>{{item.NAME}}</view>
						</view>
					...
					<scroll-view class="list" scroll-y @scroll="scroll" @scrolltolower="loadMore(index1)" @touchstart="touchmove($event,index1)">
						<view>
							<view v-if="tab.lunboImgs.length>0">
								<swiper indicator-dots="true" v-if="set_page.lunbo==1">
			...
			randomfn() {
				...
				for (let i = 0, length = this.tabBars.length; i < length; i++) {
					...
					if (i < 1) {
						...
						// 	aryItem.lunboImgs = res.data.res;
						// });
						// get_activity().then(res => {
						// 	// console.log(JSON.stringify(res.data.res));
						// 	aryItem.activity = res.data.res;
						// });
						// get_goods_1().then(res => {
						// 	console.log(res.data.res);
						// 	aryItem.goodsJpbk = res.data.res;
						// });
						// get_goods_2().then(res => {
						// 	console.log(res.data.res);
						// 	aryItem.goodsTmcs = res.data.res;
						// });
						// get_goods_3().then(res => {
						// 	console.log(res.data.res);
						// 	aryItem.goodsTmgj = res.data.res;
						// });
