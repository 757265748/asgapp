	...
	<view class="center">
		...
		<view class="center-list">
			<view class="center-list-item border-bottom">
				...
				<text @click="goLogin" class="list-text">帐号管理</text>
...
<script>
import registerLp1 from '@/api/user';
export default {
	data() {
		return {
			login: false,
			...
			if (plus.device.vendor == 'Apple') {
				...
				bcPlugin.login(result => {
					if (result.type == 'success') {
						...
						if (this.logintext) {
							uni.request({
								url: 'http://39.108.215.49:8009/GetRelationIdWebService.asmx/GetRelationId',
								data: {
									code: this.logintext.topAuthCode,
									phone: this.phone
								},
								success: res => {
