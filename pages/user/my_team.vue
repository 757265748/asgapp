		...
		<swiper :current="tabIndex" class="swiper-box" :duration="300" @change="changeTab">
			<swiper-item v-for="(tab,index1) in newsitems" :key="index1">
				<scroll-view class="list" scroll-y @scrolltolower="loadMore(index1)">
					...
					<view v-if="tabIndex==2" class="item uni-flex uni-row between" v-for="(item,index) in team_list2" :key="'key'+index">
						<view class="uni-flex uni-row">
							<view class="flex_1" style="display: flex;align-items:center;margin-right: 10upx;">
	...
	export default {
		...
		methods: {
			searchTeam(v) {
				...
				let list = this.allData.team_list1;
				switch (this.tabIndex) {
					case 0:
						list = this.allData.team_list;
						break;
					case 1:
						list = this.allData.team_list1;
						break;
					case 2:
					console.log(2);
						list = this.allData.team_list2;
						break;
				}
				console.log(list);
				let listResult = [];
			...
			getTeamList(TeamId) {
				GetTeamEmp({
					...
					if (res.data.code == 200) {
						...
						this.allData.team_list1 = res.data.result;
				...
				GetTeamEmp({
					...
					if (res.data.code == 200) {
						...
						this.allData.team_list2 = res.data.result;
						this.team_count += res.data.result.length;//统计个数
					}
				})
			},
			addData(e) {
