...
router.post('/post/pc/operatordetail-select', async (ctx) => {
    ...
    let results = await _query(`Call operatorInfo()`);
...
//获取所有合伙人的信息
router.post('/post/pc/memberlistall', async (ctx) => {
    let result = await msql.query(`CALL operatorUserInfoAll()`)
...
router.post('/post/pc/operatordetail-insert', async (ctx) => {
    ...
    let pwd = md5.md5(data.password + md5.MD5_SUFFIX);
    let nowdate = data.EffectiveTime.slice(0, 10)
    console.log("data", data);
    let ret = await msql.query(`CALL insertoperator('${data.OperatorCode}','${data.OperatorName}','${data.OperatorPID}','${data.phone}','${pwd}','${InviteCode}','${nowdate}','${data.zfb}')`)
...
router.post('/post/pc/upgradelevel', async (ctx) => {
    let { phone, jurisdiction } = ctx.request.body
    console.log('等级', jurisdiction);
    let type = jurisdiction == '团长' ? 2 : 3
