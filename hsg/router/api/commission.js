...
router.get('/get/yj/txlist', async (ctx) => {
    let { phone } = ctx.query
    let data = await msql.query(`Call Getcommissiontrans('${phone}')`)
    data = data && data[0]
    let status = new Map().set(0, '审核中').set(1, '已审核').set(2, '审核失败')
...
//按支付宝编号查询
router.get('/get/yj/idsearch', async (ctx) => {
    let { user } = ctx.query
    let data = await msql.query(`SELECT * FROM commissiontrans where user ='${user}'`)
    let status = new Map().set(0, '审核中').set(1, '已审核').set(2, '审核失败')
    for (let item of data) {
        item.TransDate = item.TransDate.toLocaleString();
        item.StautsType = status.get(item.StautsType)
...
router.get('/get/yj/search', async (ctx) => {
    let { date } = ctx.query
    console.log(date)
    let data = await msql.query(`SELECT * FROM commissiontrans where TransDate like '%${date}%'`)
    let status = new Map().set(0, '审核中').set(1, '已审核').set(2, '审核失败')
    for (let item of data) {
        item.TransDate = item.TransDate.toLocaleString();
        item.StautsType = status.get(item.StautsType)
...
//更新提现状态
router.post('/post/yj/upstatus', async (ctx) => {
    let { id } = ctx.request.body
    console.log(id)
    await msql.query(`CALL updatecommissiontrans(${id})`)
    ctx.body = {
        code: 200,
        msg: 'suceess'
    }
