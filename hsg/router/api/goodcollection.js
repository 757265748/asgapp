...
router.post('/post/good/goodcollection/add', async (ctx) => {
	...
	console.log("num_iid:"+data.num_iid);
...
router.post('/post/good/goodcollection/exisit', async (ctx)=>{
	let request=ctx.request.body;
	console.log(request);
	var result = await msql.query("select * from good_collection where phone="+request.phone);
	ctx.body={
		code:200,
		result
	}
...
router.get('/get/good/goodcollection/getRelationId', async (ctx) => {
    let user = ctx.query.user
//    let result = await msql.query(`select *from user where phone = ${user}`);
    let result = await msql.query(`select *from user where phone = 18050784265`);
    console.log(result);
    if (!result) {
        return ctx.body = {
            code: 100,
